...
const int NCPS  = 20;         //cps buffer count (4 seconds)
const int NCPM  = 30;         //cpm buffer count (2 minutes)
const int NTEMP = 50;         //temp buffer count
const int NHUM = 50;          //hum buffer count
...
const char * wpa    = "<WPA Key>";
byte server[]       = {192, 168, 1, 10};
const char * cidStr = "rad";
const int cid       = 1;

WizClientAuto wiz;
PubSubClient mqClient(server, 1883, NULL, wiz);
 ...
 */
void sendResult(unsigned int cnt) {
  //send result
  sendTick++;
  if (sendTick >= nsend) {
    sendTick = 0;
